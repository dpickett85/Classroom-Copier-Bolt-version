 ...
 *
 * **The click redirects through the API** (UX 1b). The API sets the OAuth state
 * cookie during that top-level request, which keeps it available when Google
 * sends the browser back to the callback.
 *
...
import { API_BASE_URL, confirmSession, isAbortError } from '../../lib/api-client'
...
export function AuthFlow({
  ...
  const signIn = useCallback(() => {
    ...
    setBusy(true)
    setError(null)
    // Start OAuth with a top-level request so the state cookie is first-party
    // when the browser returns from Google.
    redirect(`${API_BASE_URL}/api/auth/google/start`)
  }, [redirect])
